...
MAX_SUSPENDPACKETS = 50

# Threshold of the packet suspends thread count.
ARP_REPLY_TIMER = 2 # sec
OFP_REPLY_TIMER = 1.0 # sec
CHK_ROUTING_TBL_INTERVAL = 1800
...
class RestRouterAPI(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):super(RestRouterAPI, self).__init__(*args, **kwargs)
...
def rest_command(func):
	def _rest_command(*args, **kwargs):
		try:
			...
			status = 400
			details = e.messageexcept NotFoundError as msg:
...
class Router(dict):
	...
	def packet_in_handler(self, msg):
		...
		for p in pkt.protocols if type(p) != str)
			...
			if header_list:
				# Check vlan-tagvlan_id = VLANID_NONE
...
class VlanRouter(object):
	def __init__(self, vlan_id, dp, port_data, logger, mpls_data):
		...
		# Mplsmod:# Set flow: MPLS packets are sent to the controller
		...
		def get_data(self):
			...
			# MPLSmod: Port data for the MPLS network#prefix_data = self._get_prefix_data()
			data = {}
		...
		def set_data(self, data):
			...
			try:
				...
				# MPLSmod: set prefix-port mapping data
					...
					port = data[REST_PORT]
					tunnel_id = data[REST_TUNNEL_ID]
					prefix_id = self._set_prefix_data(prefix, port, tunnel_id)
		...
		def _set_prefix_data(self, prefix, port, tunnel_id):
			cookie = 0x800
			prefix = self.prefix_data.add(prefix, port, tunnel_id)
		...
		def _set_address_data(self, address):
			...
			self.ofctl.set_packetin_flow(cookie, priority,dl_type=ether.ETH_TYPE_IP,
	...
	def _set_defaultroute_drop(self):
		...
		priority = self._get_priority(PRIORITY_DEFAULT_ROUTING)outport = None # for drop
	...
	def _delete_routing_data(self, route_id, waiters):
		...
		# Get all flow.
		msgs = self.ofctl.get_all_flow(waiters)delete_list = []
		for msg in msgs:
			for stats in msg.body:
				vlan_id = VlanRouter._cookie_to_id(REST_VLANID, stats.cookie)
				if vlan_id != self.vlan_id:
	...
	def _packetin_from_mpls_network(self, msg, header_list):
		...
		if dst_ip not in self.hosts:
			...
			cookie = 0x830
			out_port = self.hosts[dst_ip].portdl_src = self.port_data[out_port].mac
	...
	def _packetin_icmp_req(self, msg, header_list):
		...
		self.ofctl.send_icmp(in_port, header_list, self.vlan_id,
				icmp.ICMP_ECHO_REPLY,icmp.ICMP_ECHO_REPLY_CODE,
	...
	def _packetin_to_node(self, msg, header_list):
		...
		if address is not None:
			...
			if route is not None:
				...
				self.logger.info(log_msg, srcip, dstip, extra=self.sw_id)
				gw_address = self.address_data.get_data(ip=route.gateway_ip)if gw_address is not None:
	...
	def send_icmp_unreach_error(self, packet_buffer):
		...
		if src_ip is not None:
			self.ofctl.send_icmp(packet_buffer.in_port,packet_buffer.header_list,
					self.vlan_id,
					icmp.ICMP_DEST_UNREACH,
...
class Port(object):
	def __init__(self, port_no, hw_addr):
		super(Port, self).__init__()self.port_no = port_no
		self.mac = hw_addr
...
class Prefix(object):
	def __init__(self, address, netmask, port, prefix_id, tunnel_id):
...
class RoutingTable(dict):
	...
	def add(self, dst_nw_addr, gateway_ip):
		...
		# Check overlapsoverlap_route = None
		if dst_nw_addr == DEFAULT_ROUTE:
...
class SuspendPacket(object):
	def __init__(self, in_port, header_list, data, timer):
		...
		self.header_list = header_listself.data = data
...
class OfCtl(object):
	...
	def send_arp(self, arp_opcode, vlan_id, src_mac, dst_mac,
		...
		if vlan_id != VLANID_NONE:
			...
			ether_proto = ether.ETH_TYPE_ARPhwtype = 1
	...
	def send_icmp(self, in_port, protocol_list, vlan_id, icmp_type,
		...
		ip_total_length = ip.header_length * 4 + ic._MIN_LENif ic.data is not None:
	...
	def send_packet_out(self, in_port, output, data, data_str=None):
		...
		# self.logger.debug('Packet out = %s', data_str, extra=self.sw_id)def set_normal_flow(self, cookie, priority):
	...
	def set_packetin_flow(self, cookie, priority, dl_type=0, dl_dst=0,
		...
		self.set_flow(cookie, priority, dl_type=dl_type, dl_dst=dl_dst,
			...
			nw_proto=nw_proto, tunnel_id=tunnel_id, actions=actions)
